...
The friendly staging hostname is `hq-staging.buildwithbpp.com`. GoDaddy DNS correctly resolves CNAME `hq-staging` to `bpp-hq-preview.pages.dev`. A July 31 activation test showed that a Pages custom hostname does not inherit the Pages Preview Access policy: the hostname returned HTTP 200 publicly. It was immediately detached from Pages and now serves no HQ content. Create and verify the custom hostname's exact-owner Access application before reattaching it. Because `buildwithbpp.com` DNS is not hosted in Cloudflare, a normal Cloudflare Worker route cannot own `/api/*`; the Pages Function and service binding are the governed same-origin boundary instead. The Worker origin allowlist already includes both exact staging hostnames.
